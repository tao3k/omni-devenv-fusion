...
//!
//! Tables:
//! - `kg_entities`: id, name, entity_type, description, source, aliases, confidence, vector, metadata
//! - `kg_relations`: id, source, target, relation_type, description, source_doc, confidence, metadata
    ...
    Arc::new(Schema::new(vec![
        ...
        Field::new(
            ...
            DataType::FixedSizeList(
                ...
                i32::try_from(dimension).unwrap_or(DEFAULT_ENTITY_DIMENSION as i32),
            ),
...
fn entities_to_batch(entities: &[&Entity], dimension: usize) -> Result<RecordBatch, ArrowError> {
    ...
    let aliases_json: Vec<String> = entities
        ...
        .collect();
    let alias_refs: Vec<&str> = aliases_json.iter().map(|s| s.as_str()).collect();
    ...
    let created_refs: Vec<&str> = created.iter().map(|s| s.as_str()).collect();
    ...
    let dim_i32 = i32::try_from(dimension).unwrap_or(1024);
...
impl KnowledgeGraph {
    ...
    pub async fn save_to_lance(&self, lance_dir: &str, dimension: usize) -> Result<(), GraphError> {
        ...
        // -- Entities --
        let entities_guard = self.entities.read().unwrap();
        let entity_refs: Vec<&Entity> = entities_guard.values().collect();
        let entity_batch = entities_to_batch(&entity_refs, dimension)
        ...
        // -- Relations --
        let relations_guard = self.relations.read().unwrap();
        let relation_refs: Vec<&Relation> = relations_guard.values().collect();
        let relation_batch = relations_to_batch(&relation_refs)
