...
impl DependencyIndexer {
    ...
    pub fn build(&mut self, verbose: bool) -> Result<DependencyIndexResult, String> {
        use rayon::prelude::*;
        ...
        let config_path = self
            ...
            .unwrap_or("assets/references.yaml".to_string());
        ...
        for (crate_name, version, source_path, symbols, is_error, error_msg) in results {
            if is_error {
                if verbose {
                ...
                self.symbol_index
                    .add_symbols(&crate_name, &source_path, &symbols);
...
fn find_files(pattern: &str, project_root: &PathBuf) -> Vec<PathBuf> {
    ...
    match output {
        ...
        Ok(_) | Err(_) => {
            ...
            if path.exists() && path.file_name().map(|n| n == "Cargo.toml").unwrap_or(false) {
                results.push(path);
            }
        }
    }
...
fn find_rs_files(source_path: &PathBuf) -> Vec<PathBuf> {
    let mut results = Vec::new();
    ...
    match output {
        ...
        Ok(_) | Err(_) => {
            log::debug!("fd not available or failed for: {:?}", source_path);
        }
    }
...
fn extract_symbols_from_crate(source_path: &PathBuf, crate_name: &str) -> Vec<ExternalSymbol> {
...
mod tests {
    ...
    fn test_build_performance() {
        ...
        // Navigate to project root from crate directory
        // omni-knowledge -> crates -> rust -> omni-dev-fusion (project root)
        let crate_root = env::current_dir().expect("Should be able to get current directory");
        let project_root = crate_root
            .parent() // omni-knowledge
            .and_then(|p| p.parent()) // crates
            .and_then(|p| p.parent()) // rust
            .and_then(|p| p.parent()) // omni-dev-fusion
            .expect("Should be able to get project root")
            .to_string_lossy()
            .to_string();

        let config_path = format!("{}/assets/references.yaml", project_root);

        let mut indexer = DependencyIndexer::new(&project_root, Some(&config_path));
