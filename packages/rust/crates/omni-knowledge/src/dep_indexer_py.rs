...
impl PyDependencyIndexer {
    ...
    fn build(&mut self, clean: bool, verbose: bool) -> String {
        let _clean = clean; // Reserved for cache clearing
        // CLI argument can force verbose; env var remains a fallback for global logging config.
        let env_verbose = std::env::var("OMNI_LOG_LEVEL")
            .map(|v| v == "DEBUG")
            .unwrap_or(false);
        match self.inner.build(verbose || env_verbose) {
