//! Zettelkasten integration for high-performance entity reference extraction.
//!
//! Provides fast regex-based extraction of entity references from zk notes.
...
/// Represents an entity reference extracted from zk note content.
...
pub fn extract_entity_refs(content: &str) -> Vec<ZkEntityRef> {
    static WIKILINK_REGEX: once_cell::sync::Lazy<Regex> = once_cell::sync::Lazy::new(|| {
        Regex::new(r"\[\[([^\]#\|]+)(?:#([^\]#\|]+))?(?:\|[^\]]+)?\]\]")
...
pub fn is_valid_entity_ref(text: &str) -> bool {
    ...
    WIKILINK_REGEX.is_match(text)
}

/// Parse a single entity reference string.
pub fn parse_entity_ref(text: &str) -> Option<ZkEntityRef> {
    static WIKILINK_REGEX: once_cell::sync::Lazy<Regex> = once_cell::sync::Lazy::new(|| {
        Regex::new(r"^\[\[([^\]#\|]+)(?:#([^\]#\|]+))?(?:\|[^\]]+)?\]\]")
            .expect("Failed to compile wikilink regex")
    });

    WIKILINK_REGEX.captures(text).map(|caps| {
        ZkEntityRef::new(
...
impl ZkRefStats {
    ...
    pub fn from_refs(refs: &[ZkEntityRef]) -> Self {
...
pub fn get_ref_stats(content: &str) -> ZkRefStats {
    ZkRefStats::from_refs(&extract_entity_refs(content))
}
