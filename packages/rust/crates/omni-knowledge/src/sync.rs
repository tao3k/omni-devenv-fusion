...
impl SyncEngine {
    ...
    pub fn discover_files(&self) -> Vec<PathBuf> {
        ...
        self._discover_recursive(&self.project_root, &mut files);
        files
    }

    fn _discover_recursive(&self, dir: &Path, files: &mut Vec<PathBuf>) {
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    ...
                    self._discover_recursive(&path, files);
    ...
    fn _should_skip_dir(&self, path: &Path) -> bool {
        ...
        for skip_dir in &self.options.skip_dirs {
            if let Some(name) = path.file_name() {
                if name.to_string_lossy() == *skip_dir {
                    return true;
                }
            }
        }
    ...
    fn _should_include_file(&self, path: &Path) -> bool {
        // Check extension
    ...
    pub fn compute_diff(&self, manifest: &SyncManifest, files: &[PathBuf]) -> SyncResult {
        ...
        for (rel_path, abs_path) in &current_files {
            if let Some(content) = std::fs::read_to_string(abs_path).ok() {
