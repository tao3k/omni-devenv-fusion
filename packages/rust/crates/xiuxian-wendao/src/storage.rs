...
use omni_vector::{METADATA_COLUMN, SearchOptions, VectorStore};
...
impl KnowledgeStorage {
    ...
    fn metadata_to_entry(
        ...
        let category = metadata
            .get("category")
            .and_then(|v| v.as_str())
            .map_or(KnowledgeCategory::Note, Self::category_from_str);
    ...
    async fn all_entries(&self) -> Result<Vec<KnowledgeEntry>, Box<dyn std::error::Error>> {
        ...
        if !options.projected_columns.contains(&METADATA_COLUMN) {
            options.projected_columns.push(METADATA_COLUMN);
        }

        let results = store
            .search_optimized(&self.table_name, vec![0.0; self.dimension], total, options)
            .await?;
    ...
    pub async fn search_text(
        ...
        let vector_store = self.vector_store().await?;
        ...
        if let Ok(fts_hits) = vector_store
