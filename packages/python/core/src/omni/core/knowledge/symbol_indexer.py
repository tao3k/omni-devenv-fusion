...
class SymbolIndexer:
    ...
    def _find_code_files(self) -> list[Path]:
        """Find all code files using config or defaults."""
        code_files = []
        ...
        if code_dirs:
            ...
            # Fallback to extensions-based discovery
    ...
    def _extract_symbols_from_file(self, file_path: Path) -> list[Symbol]:
        ...
        try:
            ...
            if result.startswith("[Error") or result.startswith("[No outline"):
                logger.debug(f"Could not extract symbols from {abs_path}")
                return []
        ...
        for file_path in code_files:
            ...
            try:
                ...
                for sym_name in old_symbols:
                    # Remove location from symbol index
