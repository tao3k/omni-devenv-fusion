...
SPECS_DIR = PROJECT_ROOT / get_setting("specs.dir", "assets/specs")
ARCHIVE_DIR = PROJECT_ROOT / get_setting("specs.archive", "assets/specs.archive")
        ...
        return json.dumps(
            {
                ...
                "spec_path": str(spec_path),
                "guidance": "Create a spec using draft_feature_spec before coding.",
            }
        )
...
async def draft_feature_spec(
    ...
    """
    Draft a new Feature Spec based on assets/specs/TEMPLATE.md.
...
async def verify_spec_completeness(spec_path: Optional[str] = None) -> str:
    ...
    if not spec_path:
        ...
        if not specs:
            return json.dumps({"success": False, "error": "No specs found. Use start_spec first."})
...
log_decision("spec_functions.loaded", {}, logger)
