"""
omni/core/orchestrator/builder.py - DynamicGraphBuilder
...
Usage:
    from omni.core.orchestrator.builder import DynamicGraphBuilder
    ...
    # Human-in-the-Loop
    builder = DynamicGraphBuilder(skill_runner, checkpoint=True)
...
class DynamicGraphBuilder:
    ...
    def __init__(
        ...
        Args:
            skill_runner: Skill runner for executing skill nodes (optional)
    ...
    def add_skill_node(
        ...
        3. Execute the skill via skill runner
        ...
        async def _skill_node_impl(state: GraphState) -> dict[str, Any]:
            ...
            if self.skill_runner is not None:
                try:
                    output = await self.skill_runner.run(skill_name, command_name, cmd_args)
                except Exception as e:
                    logger.error(
                        "skill_node_execution_failed",
                        extra={
                            "node": node_name,
                ...
                logger.warning(
                    "skill_runner_not_available",
                    extra={"node": node_name, "skill": f"{skill_name}.{command_name}"},
                )
                output = f"[Skill: {skill_name}.{command_name}]"
    ...
    def add_llm_node(
        ...
        Args:
            ...
            inference_client: LLM client to use (optional, uses default if not provided)
        ...
        async def _llm_node(state: GraphState) -> dict[str, Any]:
            ...
            if inference_client is not None:
                result = await inference_client.complete(prompt, model=model)
            elif hasattr(self.skill_runner, "inference") and self.skill_runner.inference:
                result = await self.skill_runner.inference.complete(prompt, model=model)
            else:
