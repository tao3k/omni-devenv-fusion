...
1. **Read Context**: ALWAYS call `@omni-orchestrator manage_context(action="read")` at the start of a session.
   - *Is there an active task?* → Resume it.
   - *Is the status 'Idle'?* → Ask user for instructions.
...
### 1. Config Changes → Edit nix first
...
**Pattern**: User → Orchestrator (plan) → Coder (execute) → Validate → User
