...
2. **Agent-Commit Protocol** (from `docs/how-to/git-workflow.md`):
   - Default: Make changes → Run tests → **STOP → Ask user** to commit
   - Only auto-commit if user explicitly says `"run just agent-commit"`
