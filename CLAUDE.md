...
**Tools over Rules**: Use `polish_text` MCP tool instead of memorizing rules.

- **Reference**: `design/writing-style/` - Library for humans
- **For AI**: Use `@omni-orchestrator polish_text` to enforce Rosenberg/Mechanics standards
- **Key Rules** (for human reference):
  - Concrete First: Example → Theory (Feynman)
  - Strip Clutter: Replace "utilize" with "use"
  - Active Voice: "The script builds" not "App is built"
  - Few-Shot: Show `Input -> Output` examples
  - Explicit Context: Avoid floating pronouns ("it", "that")
...
### ⚠️ Default Rules (Memorized)
1. **Config changes → Edit nix first.** Files like `cog.toml`, `.conform.yaml` are generated by nixago from `lefthook.nix`. Never edit generated files directly.

2. **Git Operations → Use `git_ops` tools**
   - **All commits**: Use `@omni-orchestrator smart_commit`
   - Rules are auto-loaded from `docs/how-to/git-workflow.md` (cached per session)
   - If rejected, read the error message and retry

3. **Feature Implementation → Use `product_owner` tools**
   - **Assess complexity**: `@omni-orchestrator assess_feature_complexity`
   - **Verify alignment**: `@omni-orchestrator verify_design_alignment`
   - **Get requirements**: `@omni-orchestrator get_feature_requirements`
