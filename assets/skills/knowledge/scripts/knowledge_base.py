...
def save(
    ...
    # Ensure knowledge directory exists (using get_harvest_dir for SSOT)
    knowledge_dir = get_harvest_dir(category)
...
def search(
    ...
    Returns:
        Markdown-formatted search results (truncated for display)
    """
    results = []
    ...
    for cat in categories:
        knowledge_dir = get_harvest_dir(cat)
    ...
    # Truncate results for display (max 5 entries shown)
    display_results = results[:5]
    total_count = len(results)

    markdown = f'# Search Results: "{query}"\n\n'
    markdown += f"Found {total_count} matching entries (showing {len(display_results)}):\n\n"
    ...
    if total_count > 5:
        markdown += f"*... and {total_count - 5} more results*\n"
...
def _update_index(category: str, title: str, tags: list[str], path: str) -> None:
    ...
    index_dir = harvest_dir.parent / "index"
