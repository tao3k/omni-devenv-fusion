...
def generate_xml_guide(config: dict) -> str:
    ...
    purpose = description.split('\n\n')[0].strip() if '\n\n' in description else description.split('\n')[0].strip()
    ...
    if annotations.get("destructiveHint"):
        xml.append('    <behavior>Destructive: Performs permanent changes. Confirm with user if unsure.</behavior>')
    xml.append('  </context>')
    ...
    xml.append(f'      <reasoning>This call triggers the tool with standard parameters for its primary purpose.</reasoning>')
    xml.append('    </scenario>')
    xml.append('  </scenarios>')
