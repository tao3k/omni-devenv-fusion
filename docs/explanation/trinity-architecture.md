# Trinity Architecture (v4.0 - ODF-EP v6.0)

> **Philosophy:** "Thin Agent, Fat Kernel, Rust Foundation"
> **Performance:** Pydantic V2 + Orjson = 10x faster serialization

The Trinity Architecture divides the Omni-Dev Fusion system into **four strict layers**, ensuring high performance, clear separation of concerns, and massive scalability.
...
| Component                    | Purpose                                                    |
| ---------------------------- | ---------------------------------------------------------- |
| **OrjsonModel**              | Base class with 10x faster JSON serialization via `orjson` |
| **@skill_command**           | Decorator that generates Pydantic V2 schemas automatically |
| **\_generate_tool_schema()** | Reflection-based OpenAPI schema generation                 |
| **CommandResult[T]**         | Generic typed result with @computed_field serialization    |

### Foundation API

```python
from omni.foundation.api import skill_command, CommandResult
from omni.foundation.api.types import OrjsonModel

# Decorator generates input_schema automatically
@skill_command(
    name="git_status",
    category="read",
    inject_root=True
...
def status(project_root: Path) -> CommandResult[dict]:
    """Get git status."""
    return CommandResult(success=True, data={"branch": "main"})

# OrjsonModel provides ultra-fast serialization
class MyModel(OrjsonModel):
    name: str
    value: int

json_bytes = MyModel(name="test", value=42).model_dump_json_bytes()
```
...
| Component               | Purpose                                             |
| ----------------------- | --------------------------------------------------- |
| **Kernel**              | Singleton orchestrator with lifecycle management    |
