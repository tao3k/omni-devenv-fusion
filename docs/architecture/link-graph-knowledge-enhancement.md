...
**Implementation**: `recall.py::_apply_dual_core_recall_boost` computes fusion weights once and threads them through both LinkGraph proximity (Bridge 1a) and KG entity recall (Bridge 1b).
...
| DualCoreSearch fusion logic (intent-aware merge)           | `assets/skills/knowledge/scripts/search/hybrid.py`                       |
...
| Bridge 1a: LinkGraph proximity boost (fusion-scaled)       | `dual_core/link_graph_proximity.py`                                      |
...
| ------------------------------ | ------------------------------------------------------- | ---------------------------------------------- |
| Bridge 1a (LinkGraph → Recall) | `test_dual_core.py::TestZkLinkProximityBoost`           | 5                                              |
| Bridge 2 (LanceDB → LinkGraph) | `test_dual_core.py::TestBuildVectorSearchForZk`         | 4                                              |
| Bridge 3 (LinkGraph → Router)  | `test_dual_core.py::TestEnrichSkillGraphFromZk`         | 5                                              |
