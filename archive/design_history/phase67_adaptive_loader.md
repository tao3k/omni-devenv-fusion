# Phase 67: The Adaptive Loader (Infinite Toolbox Architecture)

> **Status**: Implemented | **Date**: 2026-01-14 | **Author**: Claude

## Overview

Phase 67 transforms the Agent from a "preload everything" model to an "infinite toolbox" architecture. The Agent can now manage an unlimited skill library while keeping memory footprint low through:
...
Previously, LLM only knew about pre-loaded skills. As the skill library grew, we faced:
...
### Implementation (`context_builder.py`)
...
# In context building flow
loaded_tools = get_loaded_tools()  # From MCP
...
# LLM receives all_tools with both loaded + ghost schemas
```
...
def load_skill(self, skill_path: Path, *, reload: bool = False) -> Skill | None:
    ...
    # Phase 67: Register in LRU and enforce memory limit
...
async def run(self, skill_name: str, ...):
    # After successful JIT load
    self._touch_skill(skill_name)  # Mark as recently used
    # ... execute ...
```
...
Phase 36.5 hot reload included:

- `_validate_syntax()` - Pre-load file validation (double IO)
- Complex transactional rollback logic
- Over-engineered safety for developer errors

### Solution

Simplify to fail-fast. Let Python's import mechanism catch errors. Rely on Meta-Agent for self-repair.
...
async def test():
    ...
    print(f'Ghost tools: {len(ghosts)}')
    for g in ghosts:
...
async def test():
    sm = SkillManager()
    print(f'Initial: {len(sm._skills)} skills')
    print(f'Max: {sm._max_loaded_skills}')
    print(f'Pinned: {sm._pinned_skills}')

    await sm.run('git', 'status', {})  # JIT loads
    print(f'After JIT: {len(sm._skills)} skills')
    print(f'LRU: {sm._lru_order}')
...
| `packages/python/agent/src/agent/core/skill_manager/jit_loader.py` | Phase 63 JIT Loader (existing, separate) |
