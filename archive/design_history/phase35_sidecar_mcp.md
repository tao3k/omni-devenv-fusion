# Phase 35: Sidecar Pattern + Pure MCP Server

> **Status**: Implemented (Partially legacy - Sidecar still active, Pure MCP superseded by newer versions)
> **Date**: 2024-XX-XX
> **Related**: Phase 29 (Trinity + Protocols), Phase 36 (Trinity v2.0)

## Overview

Phase 35 introduced two key architectural improvements:

1. **Phase 35.2: Sidecar Execution Pattern** - Heavy dependency isolation
2. **Phase 35.3: Pure MCP Server** - Protocol-native MCP implementation
...
Skills with heavy dependencies (crawl4ai, playwright, etc.) pollute the main agent environment:
...
Using FastMCP adds unnecessary overhead and dependencies:

```python
# FastMCP approach
from fastmcp import FastMCP

mcp = FastMCP("omni-agent")
...
- FastMCP abstraction overhead
- Additional dependency to maintain
- Less control over protocol handling

### The Solution: Pure MCP Server

```python
# Pure MCP approach (Phase 35.3)
from mcp.server import Server
from mcp.types import Tool, TextContent

# Create pure MCP Server
...
async def handle_list_tools() -> list[Tool]:
    ...
    for skill_name in manager.list_loaded():
        for cmd_name in manager.get_commands(skill_name):
            ...
            if skill and cmd_name in skill.commands:
                cmd = skill.commands[cmd_name]
                tools.append(Tool(
                    name=f"{skill_name}.{cmd_name}",
                    description=cmd.description,
                    inputSchema={
                        "type": "object",
                        "properties": get_cmd_params(cmd),
                    },
                ))
    return tools
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    result = await get_skill_manager().run(skill_name, cmd_name, arguments)
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        return [TextContent(
            type="text",
            text=f"Error: Skill not found: {skill_name}"
        )]
    except CommandNotFoundError:
        return [TextContent(
            type="text",
            text=f"Error: Command not found: {cmd_name}"
        )]
...
| File                             | Change                              |
| -------------------------------- | ----------------------------------- |
| `common/isolation.py`            | NEW: Sidecar execution module       |
| `agent/mcp_server.py`            | NEW: Pure MCP Server implementation |
| `agent/core/registry/`           | Refactored for modular structure    |
| `assets/skills/*/pyproject.toml` | Added for sidecar skills            |

## Related Specs

- `assets/specs/phase29_trinity_protocols.md`
- `assets/specs/phase36_trinity_v2.md`
- `assets/specs/phase50_cca_navigation.md` (uses pure MCP)
